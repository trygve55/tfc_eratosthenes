...
public abstract class EqualEarth implements MapProjection {
    ...
    private static final float MAGIC_NUMBER = 1.317362725f; // invertY(MAGIC_NUMBER) = PI/2 = Math.toRadians(90)
    private static final float MAGIC_NUMBER_2 = 1.1607025861777f; // getRelativeWidthAtLatitude(0) * MAGIC_NUMBER_2 = 1
    ...
    public int getHalfCircumferenceAtLatitude(float latitude) {
        return (int) (getRelativeWidthAtLatitude(latitude) * getHalfMeridian() * MAGIC_NUMBER_2 * getWidthToHeightRatio());
    }
    ...
    public float getLatitude(float equatorDistance) {
        return (float) Math.toDegrees(invertEqualEarthLatitude( equatorDistance/getHalfMeridian() * MAGIC_NUMBER));
    }
